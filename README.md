...
| <a name="input_allow_ssl_requests_only"></a> [allow\_ssl\_requests\_only](#allow\_ssl\_requests\_only) | Set to `true` to require requests to use Secure Socket Layer (HTTPS/SSL). This will explicitly deny access to HTTP requests | `bool` | `false` | no |
