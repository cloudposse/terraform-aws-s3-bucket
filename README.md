...
| <a name="input_log_delivery_service_principal_enabled"></a> [log\_delivery\_service\_principal\_enabled](#input\_log\_delivery\_service\_principal\_enabled) | When set to `true` it will add `delivery.logs.amazonaws.com` service principal to the bucket policy | `bool` | `false` | no |
