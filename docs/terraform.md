...
| <a name="input_cloudtrail_bucket_policy_enabled"></a> [cloudtrail\_bucket\_policy\_enabled](#input\_cloudtrail\_bucket\_policy\_enabled) | Whether or not to allow writing Cloudtrail logs to the S3 bucket by applying the appropriate bucket policy | `bool` | `false` | no |
