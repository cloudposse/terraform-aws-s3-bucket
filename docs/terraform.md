...
| <a name="input_cloudtrail_bucket_policy_enabled"></a> [cloudtrail\_bucket\_policy\_enabled](#input\_cloudtrail\_bucket\_policy\_enabled) | Setting this to true allows cloud trail service to write to the log by applying the appropriate bucket policy | `bool` | `false` | no |
