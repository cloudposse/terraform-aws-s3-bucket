...
| <a name="input_policy"></a> [policy](#input\_policy) | JSON string representation of the bucket policy for this bucket as list of string (0 or 1 items).<br><br>If empty, no custom bucket policy document will be used. If the list contains a single document, a custom IAM policy will<br>be created and attached to the bucket.<br><br>Can also be a plain string, but that use is DEPRECATED because of Terraform issues (see: https://github.com/hashicorp/terraform/issues/26755#issuecomment-719103775).<br><br>Note that if the policy document is not specific enough (but still valid), Terraform may view the policy as constantly<br>changing in a terraform plan. In this case, please make sure you use the verbose/specific version of the policy. | `any` | `[]` | no |
