...
data "aws_iam_policy_document" "replication" {
  ...
  statement {
    ...
    resources = toset([for rule in var.replication_rules : "${try(length(rule.value.destination.bucket), 0) > 0 ? rule.value.destination.bucket : var.s3_replica_bucket_arn}/*"])
  }
}
